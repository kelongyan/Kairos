...
- Agent run 和 Agent step 持久化，支持 `/agent-runs` 创建、列表和详情。
- 前端三栏工作区：知识库、文档列表、Chat/Agent 模式、引用、retrieval trace、Agent step trace 和 Agent run 历史回看。
...
- SSE 流式输出和多轮会话。
- 评测 API、统计看板。
- 热门问题、无答案问题、低质量引用等知识运营清单。
- LangGraph 编排替换、外部搜索工具和 Knowledge Operations Agent。
...
```text
Agent run 过滤与回看
  -> Knowledge Operations Agent 草稿建议
  -> 审计日志
  -> 知识运营清单
  -> 评测 API
  -> 最小 auth/RBAC
