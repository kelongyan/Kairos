...
- Evidence Pack 构建。
- `/chat` 返回 answer、citations 和 retrieval trace。
- 前端三栏工作区：文档列表、Chat、引用与 trace 面板。
...
- 用户、登录、RBAC、审计日志。
- SSE 流式输出和多轮会话。
- Trace 持久化、评测 API、统计看板。
- 用户反馈、热门问题、无答案问题、低质量引用等知识运营能力。
- Multi-Agent 编排。
...
这一步完成后，系统才真正从“单文档 RAG demo”进入“可落地的团队知识库产品”。
